...
class HParam:
    ...
    num_layers = 3
    seq_length = 20
    log_dir = './model/zhoujielun'
    metadata = './metadata/zhoujielun.tsv'
    gen_num = 500   # how many chars to generate

    def __init__(self, singer):
        self.log_dir = './model/' + singer
        self.metadata = './metadata/' + singer + '.tsv'
...
def sample(data, model, args, start):
...
def gener(infer, singer, start):
    args = HParam(singer)
    data = DataGenerator('./data/' + singer + '.txt', args)
    model = Model(args, data, infer=infer)
    return sample(data, model, args, start)


if __name__ == '__main__':
    lyrics = gener(1, 'xusong', '你不需要我')
    print(lyrics)
